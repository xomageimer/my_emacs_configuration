    ...
    (let ((line (buffer-substring-no-properties (point) (line-end-position)))
      ...
      (when (string-match "\\([0-9]+\\):\\([0-9]+\\) - \\([0-9]+\\):\\([0-9]+\\)" line)
        (let* ((hours1 (string-to-number (match-string 1 line)))
               ...
               (next-day (and (< hours2 hours1) t))
               (adjusted-hours2 (if next-day (+ hours2 24) hours2))
               (total-minutes (+ (* 60 (- adjusted-hours2 hours1)) (- minutes2 minutes1)))
...
(defun my/ws-butler-clean-on-save ()
  "Run `ws-butler-clean-region` before saving the buffer."
