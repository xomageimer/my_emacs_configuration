...
(defun display-json-files-map ()
  "Display the map generated by `parse-json-files-in-directory` for DIRECTORY."
  (interactive)
  (let ((result (parse-json-files-in-directory "~/Downloads/Mython-master/build/.cmake/api/v1/reply"))
        (buf (get-buffer-create "*JSON Files Map*")))
    (with-current-buffer buf
      (erase-buffer)
      (insert (prin1-to-string (cdr result))))
    (display-buffer buf)))

(defun print-target-for-source (json-hash source)
  "Print the target for the given source file."
  (maphash
