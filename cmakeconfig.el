...
(defun build-and-run-project (target)
  ...
  (let ((compilation-buffer-name-function (lambda (mode) (concat "*Running " target "*")))
    ...
    (compile compile-command)
    (setq compilation-buffer (get-buffer (funcall compilation-buffer-name-function 'compilation-mode)))
    (set-process-sentinel (get-buffer-process (compilation-find-buffer))
                          `(lambda (process event)
                             (run-sentinel process event ,target ,args ,compilation-buffer)))))

(defun run-sentinel (process event target args buffer)
  (when (eq (process-status process) 'exit)
    (let ((target-path (gethash target targets_by_path)))
      (async-shell-command (concat (my/create-build-dir) "/" target-path " " args) (get-buffer-create buffer)))))
...
(defun run-boost-test-case ()
  ...
  (let ((current_file (get-project-relative-file-name))
    ...
    (set-process-sentinel (get-buffer-process (compilation-find-buffer))
                          `(lambda (process event)
                             (run-sentinel process event ,target ,test_args ,compilation-buffer)))))
